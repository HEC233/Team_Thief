...
public class NewGameEventSystem : MonoBehaviour
{
    private Dictionary<string, PS.Event.Event[]> _eventMap;
    ...
    private void Start()
    {
        GameManager.instance.AddMapStartEventListener(MapStartEvent);
    }

    public void EstablishEventMap()
    {
        _eventMap = new Dictionary<string, PS.Event.Event[]>();
        var _events = Resources.LoadAll<PS.Event.Event>("Event");
        Dictionary<string, int> sceneCount = new Dictionary<string, int>();
        Dictionary<string, int> sceneIndex = new Dictionary<string, int>();

        foreach (var _event in _events)
        {
            if(_event.eventScene == null)
            {
                continue;
            }
            string[] scenes = _event.eventScene.Split(',');
            foreach (var sceneName in scenes)
            {
                if (!sceneCount.ContainsKey(sceneName))
                {
                    sceneCount.Add(sceneName, 0); 
                    sceneIndex.Add(sceneName, 0);
                }
                sceneCount[sceneName]++;
            }
        }

        foreach (string eventName in sceneCount.Keys)
        {
            _eventMap.Add(eventName, new PS.Event.Event[sceneCount[eventName]]);
        }
        ...
        foreach (var _event in _events)
        {
            ...
            foreach (var sceneName in scenes)
            {
                _eventMap[sceneName][sceneIndex[sceneName]++] = _event;
            }
        }
    }
    ...
    public void MapStartEvent()
    {
        ...
        {
            foreach (var _event in _eventMap[curScene])
            {
                StartCoroutine(ProcessEvent(_event));
            }
        }

        _player = GameManager.instance.ControlActor.GetUnit().gameObject;
        bRestrict = false;
    }
    ...
    IEnumerator ProcessEvent(PS.Event.Event data)
    {
